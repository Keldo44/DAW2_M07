    ...
    <div class="container mx-auto mt-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form action="{{ route('products.update', $product->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $product->name) }}" class="mt-1 p-2 border rounded w-full">
                @error('name')
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" class="mt-1 p-2 border rounded w-full">{{ old('description', $product->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="stock" id="stock" value="{{ old('stock', $product->stock) }}" class="mt-1 p-2 border rounded w-full">
                @error('stock')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="price" id="price" value="{{ old('price', $product->price) }}" class="mt-1 p-2 border rounded w-full">
                @error('price')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
